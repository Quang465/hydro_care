// card.js (time-scale realtime chart, last 10 minutes)
...
// Chuẩn thông số
const fishStandards = {
  "fish1": { temperature: [28, 30], tds: [10, 300], ph: [7, 9], turbidity: [0, 40] },
  "fish2": { temperature: [27, 30], tds: [10, 300], ph: [6.5, 7.5], turbidity: [0, 40] },
  "fish3": { temperature: [28, 32], tds: [10, 300], ph: [5.5, 6.8], turbidity: [0, 40] }
};
// đánh giá thông số
...
async function loadHistory() {
  ...
  [tdsChart, turbidityChart, tempChart, phChart].forEach(c => { c.data.datasets[0].data = []; });
...
function subscribeRealtime() {
  ...
  supabase
    ...
    .on("postgres_changes", { event: "INSERT", schema: "public", table: "sensors_data", filter: `device_id=eq.${espId}` },
      (payload) => {
  ...
  try {
    const { data, error } = await supabase
      ...
      .upsert(
        ...
        { onConflict: ["device_id"] }   // ✅ phải là device_id
      );
